...
public class SysmlDiagramActionBarContributor extends
	...
	class EditorAction extends Action {
		...
		public void run() {
			...
			int contentSize = editor.getDiagram().getElement().eContents()
					.size();
			...
			for (int i = 0; i < contentSize; i++) {
				reportContent = reportContent
						...
						+ i
						+ " is "
						+ editor.getDiagram().getElement().eContents().get(i)
								.getClass().toString() + "\n";
			}
			...
			for (int i = 0; i < diagramPartSize; i++) {
				...
				List sourceConnectionsList = ((ShapeNodeEditPart) childrenList
						.get(i)).getSourceConnections();
				List targetConnectionsList = ((ShapeNodeEditPart) childrenList
			...
			try {
				...
				compilerResult = CompilerProxy.sendExpression(command,
						showInConsole);
				result = compilerResult.getResult();
			} catch (Exception e) {

			}
			MessageDialog.openInformation(shell, "ModelicaML menu", "testing "
					+ label
					//  +" \n "+ diagramEditPart
	...
	public SysmlDiagramActionBarContributor() {
		...
		inheritanceDiagramHandler = new SysmlInheritanceDiagramAction(
				"Inheritance Diagram", SysmlInheritanceDiagramAction.FROM_CLASS);
