 ...
 */
/**
 * A variable in a GDB stack frame
 */
...
//		fReplyReader = frame.getGDBDebugTarget().getReplyReader();
	...
	class ComponentVariable extends GDBDebugElement implements IVariable
	{
		...
		public ComponentVariable(GDBDebugTarget t, String name, IValue value, String type)
		{
			super(t);
	...
	public IValue buildGDBValue(DefaultMutableTreeNode root)
	{
		...
		{
			return new GDBValue(this.getGDBDebugTarget(), value, new IVariable[0]);
		}
		...
		v.toArray(vars);
		return new GDBValue(this.getGDBDebugTarget(), value, vars);
	}
	...
	public IVariable buildComponentVariable(DefaultMutableTreeNode root)
	{
		...
		if (!kids.hasMoreElements())
		{
			return new ComponentVariable(
					this.getGDBDebugTarget(), 
					vi.rmlName, 
					new GDBValue(this.getGDBDebugTarget(), vi.rmlValue, new IVariable[0]), type);
		}
		Vector<IVariable> v = new Vector<IVariable>(); 
		while (kids.hasMoreElements())
		{
		...
		IVariable[] vars = new IVariable[v.size()];
		v.toArray(vars);
		return new ComponentVariable(this.getGDBDebugTarget(), vi.rmlName, new GDBValue(this.getGDBDebugTarget(), vi.rmlValue, vars), type);
	}
	...
	/**
	 * Sends a request to the GDB Debugger and will get one of IGDBDebugCommands.EVENT_*
	 * 
	 * @param cmd
	 *            debug command
	 ...
	 */
	protected GDBStackFrame getStackFrame() {
		return fFrame;
	}
