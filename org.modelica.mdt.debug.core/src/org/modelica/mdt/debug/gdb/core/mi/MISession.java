...
public class MISession extends Observable {
	...
	private void setup() throws MIException {
		...
		try {
			...
			if (line == null) {
				line = MDTDebugCorePlugin.getResourceString("src.MISession.Process_Terminated");
			}
	...
	public void postCommand(Command cmd, long timeout) throws MIException {
		...
		if (!fTxThread.isAlive() || !fRxThread.isAlive()) {
			throw new MIException(MDTDebugCorePlugin.getResourceString("src.MISession.Thread_Terminated"));
		}
...
//		}
		...
		if (isTerminated()) {
			throw new MIException(MDTDebugCorePlugin.getResourceString("src.MISession.Session_terminated"));
		}
			...
			while (cmd.getMIOutput() == null) {
				try {
					...
					if (cmd.getMIOutput() == null) {
						throw new MIException(MDTDebugCorePlugin.getResourceString("src.MISession.Target_not_responding"));
					}
	...
	public void terminate() {
		...
		MIGDBExit exitCommand = fMICommandFactory.createMIGDBExit();
		try {
			postCommand(exitCommand, 2000);
		} catch (MIException e) {
