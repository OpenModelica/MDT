 ...
 * A watchpoint.
 */
public class MDTWatchpoint extends MDTLineBreakpoint implements IWatchpoint {
	 ...
	 */
	public MDTWatchpoint(final IResource resource, final int lineNumber, final String functionName, final String varName, final boolean access, final boolean modification) throws CoreException {
		IWorkspaceRunnable runnable = new IWorkspaceRunnable() {
			public void run(IProgressMonitor monitor) throws CoreException {
				IMarker marker = resource.createMarker("org.modelica.mdt.debug.core.markerType.watchpoint");
				setMarker(marker);
				setEnabled(true);
	...
	public void handleEvent(String event) {
		if (event.startsWith("suspended watch")) {
			System.out.println("MDTWatchpoint RECEIVED EVENT: " + event); System.out.flush();		
			handleHit(event);
		}
	}
