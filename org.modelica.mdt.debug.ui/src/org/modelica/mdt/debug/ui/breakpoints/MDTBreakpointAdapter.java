...
/**
 * Adapter to create breakpoints in MDT files.
 */
public class MDTBreakpointAdapter implements IToggleBreakpointsTargetExtension {
	/* (non-Javadoc)
	...
	public void toggleWatchpoints(IWorkbenchPart part, ISelection selection) throws CoreException {
	    ...
	    if (variableAndFunctionName != null && part instanceof ITextEditor && selection instanceof ITextSelection) {
			...
			for (int i = 0; i < breakpoints.length; i++) {
				IBreakpoint breakpoint = breakpoints[i];
				if (breakpoint instanceof MDTWatchpoint && resource.equals(breakpoint.getMarker().getResource())) {
				    MDTWatchpoint watchpoint = (MDTWatchpoint)breakpoint;
			...
			// create watchpoint
			MDTWatchpoint watchpoint = new MDTWatchpoint(resource, lineNumber + 1, fcn, var, true, true);
	 ...
	 * 
	 * @param document MDT source file
