	...
	public FoldingActionGroup(final ITextEditor editor, ITextViewer viewer) {
		...
		fRestoreDefaults= new FoldingAction(FoldingMessages.getResourceBundle(), "Projection.Restore.") { //$NON-NLS-1$
			public void run() {
				if (editor instanceof ModelicaEditor) {
					...
					modelicaEditor.resetProjection();
				}
			}
		};
		...
		fCollapseMembers= new FoldingAction(FoldingMessages.getResourceBundle(), "Projection.CollapseMembers.") { //$NON-NLS-1$
			public void run() {
				if (editor instanceof ModelicaEditor) {
					ModelicaEditor modelicaEditor= (ModelicaEditor) editor;
					modelicaEditor.collapseMembers();
				}
			}
		};
		fCollapseMembers.setActionDefinitionId(IModelicaEditorActionDefinitionIds.FOLDING_COLLAPSE_MEMBERS);
		...
		fCollapseComments= new FoldingAction(FoldingMessages.getResourceBundle(), "Projection.CollapseComments.") { //$NON-NLS-1$
			public void run() {
				if (editor instanceof ModelicaEditor) {
					ModelicaEditor modelicaEditor= (ModelicaEditor) editor;
					modelicaEditor.collapseComments();
				}
			}
		};
		fCollapseComments.setActionDefinitionId(IModelicaEditorActionDefinitionIds.FOLDING_COLLAPSE_COMMENTS);
