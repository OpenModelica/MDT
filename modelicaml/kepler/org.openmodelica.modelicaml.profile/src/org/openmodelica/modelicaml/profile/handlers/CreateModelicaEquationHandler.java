...
/**
 * The Class CreateModelicaEquationHandler.
 */
public class CreateModelicaEquationHandler extends AbstractHandler {
...
private String stereotypePath = Platform.getResourceString(Activator.getDefault().getBundle(), "%p_path_ModelicaBehaviorConstructs");
	...
	/** The stereotyep name. */
	private String stereotyepName = Platform.getResourceString(Activator.getDefault().getBundle(), "%s_equation");
...
//### START: adapt it for a new command handler
				if (selectedElement instanceof Class) {
					...
					OB = (OpaqueBehavior) ((org.eclipse.uml2.uml.Class)selectedElement).createOwnedBehavior(stereotyepName,UMLPackage.Literals.OPAQUE_BEHAVIOR);
					stereotype = OB.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				...
				if (stereotype == null) {
					...
					MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML stereotype " + stereotyepName +" to " + OB.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
				}
				else {
					OB.applyStereotype(stereotype);
				}
				
				// locate in Papyrus
				if ( OB != null) {
					EditorServices.locateInModelExplorer(OB, true);
				}
//### END: adapt it for a new command handler
