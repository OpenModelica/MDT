...
/**
 * The Class CreateModelicaOperatorHandler.
 */
public class CreateModelicaOperatorHandler extends AbstractHandler {
...
//### START: adapt it for a new command handler
				if (selectedElement instanceof Class) {
					...
					clas = (Class) ((org.eclipse.uml2.uml.Class)selectedElement).createNestedClassifier(stereotyepName, UMLPackage.Literals.CLASS);
					stereotype = clas.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				else if (selectedElement instanceof Package) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					clas = ((org.eclipse.uml2.uml.Package)selectedElement).createOwnedClass(stereotyepName, false);
					stereotype = clas.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				...
				if (stereotype == null) {
					...
					MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML stereotype " + stereotyepName +" to " + clas.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
				}
				else {
