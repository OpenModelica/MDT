...
/**
 * The Class CreateModelicaFunctionHandler.
 */
public class CreateModelicaFunctionHandler extends AbstractHandler {
...
private String stereotypePath = Platform.getResourceString(Activator.getDefault().getBundle(), "%p_path_ModelicaClassConstructs");
	...
	/** The stereotyep name. */
	private String stereotyepName = Platform.getResourceString(Activator.getDefault().getBundle(), "%s_function");
	...
	protected Command getCommand(TransactionalEditingDomain editingDomain) {
		...
		Command command = new RecordingCommand(editingDomain) {
			...
			protected void doExecute() {
				...
				FunctionBehavior clazz = null;
				Stereotype stereotype = null;
				
//### START: adapt it for a new command handler
				if (selectedElement instanceof Class) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					clazz = (FunctionBehavior) ((org.eclipse.uml2.uml.Class)selectedElement).createOwnedBehavior(stereotyepName, UMLPackage.Literals.FUNCTION_BEHAVIOR);
					stereotype = clazz.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				else if (selectedElement instanceof Package) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					clazz = (FunctionBehavior) ((org.eclipse.uml2.uml.Package)selectedElement).createPackagedElement(stereotyepName, UMLPackage.Literals.FUNCTION_BEHAVIOR);
					stereotype = clazz.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				
				else if (selectedElement instanceof FunctionBehavior) {
					clazz = (FunctionBehavior) ((org.eclipse.uml2.uml.FunctionBehavior)selectedElement).createOwnedBehavior(stereotyepName, UMLPackage.Literals.FUNCTION_BEHAVIOR);
					stereotype = clazz.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				else {
				...
				if (stereotype == null) {
					...
					MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML stereotype " + stereotyepName +" to " + clazz.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
				}
				else {
