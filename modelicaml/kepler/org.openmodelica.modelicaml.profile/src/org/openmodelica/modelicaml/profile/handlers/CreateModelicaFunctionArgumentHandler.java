...
/**
 * The Class CreateModelicaFunctionArgumentHandler.
 */
public class CreateModelicaFunctionArgumentHandler extends AbstractHandler {
...
private String stereotypePath = Platform.getResourceString(Activator.getDefault().getBundle(), "%p_path_ModelicaCompositeConstructs");
	...
	/** The stereotyep name. */
	private String stereotyepName = Platform.getResourceString(Activator.getDefault().getBundle(), "%s_functionArgument");
...
//### START: adapt it for a new command handler
				 if (selectedElement instanceof FunctionBehavior) {
					 pr = (Parameter) ((org.eclipse.uml2.uml.FunctionBehavior)selectedElement).createOwnedParameter(stereotyepName, null);
					stereotype = pr.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				else {
				...
				if (stereotype == null) {
					...
					MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML stereotype " + stereotyepName +" to " + pr.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
				}
				else {
					pr.applyStereotype(stereotype);
				}
				
				// locate in Papyrus
				if ( pr != null) {
					EditorServices.locateInModelExplorer(pr, true);
				}
//### END: adapt it for a new command handler
