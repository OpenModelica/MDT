...
/**
 * The Class CreateModelicaEnumerationHandler.
 */
public class CreateModelicaEnumerationHandler extends AbstractHandler {
...
private String stereotypePath = Platform.getResourceString(Activator.getDefault().getBundle(), "%p_path_ModelicaClassConstructs");
	...
	/** The stereotyep name. */
	private String stereotyepName = Platform.getResourceString(Activator.getDefault().getBundle(), "%s_enumeration");
	...
	protected Command getCommand(TransactionalEditingDomain editingDomain) {
		...
		Command command = new RecordingCommand(editingDomain) {
			...
			protected void doExecute() {
				...
				org.eclipse.uml2.uml.Enumeration en = null;
				Stereotype stereotype = null;
				
//### START: adapt it for a new command handler
				if (selectedElement instanceof Class) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					en = (org.eclipse.uml2.uml.Enumeration) ((Class)selectedElement).createNestedClassifier(stereotyepName, UMLPackage.Literals.ENUMERATION);
					stereotype = en.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				else if (selectedElement instanceof Package) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					en = (org.eclipse.uml2.uml.Enumeration) ((org.eclipse.uml2.uml.Package)selectedElement).createOwnedEnumeration(stereotyepName);
					stereotype = en.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				...
				if (stereotype == null) {
					...
					MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML stereotype " + stereotyepName +" to " + en.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
				}
				else {
					en.applyStereotype(stereotype);
				}
				
				// locate in Papyrus
				if ( en != null) {
					EditorServices.locateInModelExplorer(en, true);
				}
//### END: adapt it for a new command handler
