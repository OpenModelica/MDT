...
/**
 * The Class CreatePackageHandler.
 */
public class CreatePackageHandler extends AbstractHandler {
...
private EObject selectedElement = null;
	...
	protected Command getCommand(TransactionalEditingDomain editingDomain) {
		final String commandTitle = "Create New Package(s)";
...
//### START: adapt it for a new command handler
				if (selectedElement instanceof Package) {
					...
					if (useMultipleItemsDialogForCreatingNewElements) {
						...
						EClass metaClassForNewElements = UMLPackage.Literals.PACKAGE;

						int numberOfItems = Constants.numberOfItemsInCreateNewElementsDiologs;
						
						String lblText = "Name: ";
						String defaultName = "Package";
...
//						String[] comboItems = {"Model", "Block", "Connector"};
						...
						if (dialog.getReturnCode() == 0) {
							ArrayList<String> list = dialog.getFirstColValues();
...
//				if (selectedElement instanceof Package) {
//					//TODO: Should the name get a post-fix number (e.g. name1, name2, etc.)?
//					element = (Package) ((org.eclipse.uml2.uml.Package)selectedElement).createPackagedElement("Package", UMLPackage.Literals.PACKAGE);
//				}

				else {
					System.err.println("Could not create a " + "Package" + " in " + selectedElement.getClass().getName());
				}
