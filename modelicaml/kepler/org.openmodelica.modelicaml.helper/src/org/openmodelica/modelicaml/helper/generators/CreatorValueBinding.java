...
public class CreatorValueBinding implements IRunnableWithProgress {
	...
	/*
	 * Note, all clients must provide either the instantiatedClass, or its elements to tree objects map!
	 */
		...
		if (element instanceof Property) {
			...
			if (deriveCodeHelper.getClientElement() != null ){
				...
				if (deriveCodeHelper.isMandatoryClient()) {
					...
					// if no binding could be generated and no user decision is required -> generate marker
					if (code == null && !deriveCodeHelper.isUserSelectionRequired()) {
						if ( !onlySimulate ) {
							// Generate marker 
...
//	    			long tStartAddBinding = System.currentTimeMillis(); // DEBUG ONLY
	    			
	    			ModificationManager.addComponentModification( modificationStoreLocation, componentPath, code, true);
	    			
//	    			long tEndAddBinding = System.currentTimeMillis(); // DEBUG ONLY
//					System.err.println("	" + (tEndAddBinding - tStartAddBinding)+ "ms for adding binding to " + componentPath);
