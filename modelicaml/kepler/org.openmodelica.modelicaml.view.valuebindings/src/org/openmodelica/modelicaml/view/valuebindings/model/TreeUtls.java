...
//						System.err.println("Deleting: " + dependency.getName());
	...
	public static boolean deleteElement(final TreeObject treeObject){
		final Element umlElementToDelete = treeObject.getUmlElement();
		if (umlElementToDelete != null) {
			TransactionalEditingDomain editingDomain = getTransactionalEditingDomain();
			CompoundCommand cc = new CompoundCommand("Delete mediator reference");
			Command command = new RecordingCommand(editingDomain) {
				@Override
				protected void doExecute() {
					umlElementToDelete.destroy();
				}
			};
			cc.append(command);
			editingDomain.getCommandStack().execute(cc);
		...
		MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Error", "Could not find the UML element for " + treeObject.getName());
		return false;
	}
	...
	private static TransactionalEditingDomain getTransactionalEditingDomain(){
		return EditorServices.getPapyrusEditingDomain();
	}
