...
import org.openmodelica.modelicaml.validation.util.ValidateAssigmentsInAlgorithmSections;
	 ...
	 *	Mode : Batch
	 */
	public class C49_AssignmentOfContVarsInStateMachines extends AbstractModelConstraint {

		
	public C49_AssignmentOfContVarsInStateMachines() {
	...
	public IStatus validate(IValidationContext ctx) {
		...
		if(eObj instanceof Element && ((Element)eObj).getOwner() instanceof NamedElement){
			...
			if(element instanceof Class && Utility.isElementHaveModelicaMLStereotypeApplied(element)) {
				...
				List<String> contVarsAssignedInAlgSections = v.getConinuesVariablesAssignedInStateMachines(clazz);
				
				if (contVarsAssignedInAlgSections == null) {
					...
					String vars = "";
					// Creates message 
					for (Object object : contVarsAssignedInAlgSections) {
						vars += object.toString() + ",";
					}
					
					msg = "Variable(s) "+ "'" + vars.trim().substring(0, vars.trim().length() - 1)+ "' " +
							"are must be 'discrete' because they are set in algorithm sections, state machine(s) or activities.";
