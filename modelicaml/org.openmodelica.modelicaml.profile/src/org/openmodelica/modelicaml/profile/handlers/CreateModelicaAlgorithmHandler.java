 ...
 */
public class CreateModelicaAlgorithmHandler extends AbstractHandler {
...
private String stereotypePath = Platform.getResourceString(Activator.getDefault().getBundle(), "%p_path_ModelicaBehaviorConstructs");
	...
	/** The stereotyep name. */
	private String stereotyepName = Platform.getResourceString(Activator.getDefault().getBundle(), "%s_algorithm");
	...
	protected Command getCommand(TransactionalEditingDomain editingDomain) {
		...
		Command command = new RecordingCommand(editingDomain) {
			...
			protected void doExecute() {
				...
				OpaqueBehavior OA= null;
				Stereotype stereotype = null;
				
//### START: adapt it for a new command handler
				if (selectedElement instanceof Class) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					OA = (OpaqueBehavior) ((org.eclipse.uml2.uml.Class)selectedElement).createOwnedBehavior(stereotyepName, UMLPackage.Literals.OPAQUE_BEHAVIOR);
					stereotype = OA.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
//				else if (selectedElement instanceof FunctionBehavior) {
//					OA = (OpaqueBehavior) ((org.eclipse.uml2.uml.Model)selectedElement).createOwnedType(stereotyepName, null);
//						stereotype = OA.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
//					}
				...
				if (stereotype == null) {
					...
					MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML stereotype " + stereotyepName +" to " + OA.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
				}
				else {
					OA.applyStereotype(stereotype);
				}
				
				// locate in Papyrus
				if ( OA != null) {
					EditorServices.locateInModelExplorer(OA, true);
				}
//### END: adapt it for a new command handler
