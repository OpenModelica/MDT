...
/**
 * The Class CreateSignalHandler.
 */
public class CreateSignalHandler extends AbstractHandler {
...
private EObject selectedElement = null;
	...
	protected Command getCommand(TransactionalEditingDomain editingDomain) {
		...
		CompoundCommand cc = new CompoundCommand("Signal");
...
//### START: adapt it for a new command handler
				if (selectedElement instanceof Class) {
					...
					sl = (Signal) ((org.eclipse.uml2.uml.Class)selectedElement).createNestedClassifier("Signal", UMLPackage.Literals.SIGNAL);
					//stereotype = sl.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				else {
...
//					MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML stereotype " + stereotyepName +" to " + sl.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
//				}
//				else {
//					sl.applyStereotype(stereotype);
//				}
//### END: adapt it for a new command handler
