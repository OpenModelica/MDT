 ...
 */
public class CreateValueMediatorsContainerHandler extends AbstractHandler {
...
//	private String stereotypeName = Platform.getResourceString(Activator.getDefault().getBundle(), "%s_block");
	private String stereotypeName = "ValueMediatorsContainer";	
...
//### START: adapt it for a new command handler
				if (selectedElement instanceof Class) {
					...
					clazz = (Class) ((org.eclipse.uml2.uml.Class)selectedElement).createNestedClassifier(stereotypeName, UMLPackage.Literals.CLASS);
					stereotype = clazz.getApplicableStereotype(stereotypePath+"::"+stereotypeName);
				}
				else if (selectedElement instanceof Package) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					clazz = ((org.eclipse.uml2.uml.Package)selectedElement).createOwnedClass(stereotypeName, false);
					stereotype = clazz.getApplicableStereotype(stereotypePath+"::"+stereotypeName);
				}
