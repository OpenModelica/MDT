 ...
 */
public class CreateModelicaSimulationModelHandler extends AbstractHandler {
...
private String stereotypePath = Platform.getResourceString(Activator.getDefault().getBundle(), "%p_path_ModelicaClassConstructs");
	...
	/** The stereotype name. */
	private String stereotypeName = Platform.getResourceString(Activator.getDefault().getBundle(), "%s_model");
...
//### START: adapt it for a new command handler
		final String commandTitle = "Create New Simulation Model(s)";
		...
		Command command = new RecordingCommand(editingDomain) {
			...
			protected void doExecute() {
				...
				if (useMultipleItemsDialogForCreatingNewElements) { // with dialog
					...
					String lblText = "Name: ";
					String defaultName = "SimulationModel";
...
private Class createElement(String name){
		...
		if (element != null) {
			...
			simStereotype = element.getApplicableStereotype("ModelicaML::ModelicaSimulationConstructs::Simulation");
			...
			// apply ModelicaML simulation stereotype
			if (simStereotype == null) {
				Shell shell = new Shell();
				MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML <<Simulation>> stereotype to " + element.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
			}
			else {
				element.applyStereotype(simStereotype);
			}
		}
		return element;
	}
