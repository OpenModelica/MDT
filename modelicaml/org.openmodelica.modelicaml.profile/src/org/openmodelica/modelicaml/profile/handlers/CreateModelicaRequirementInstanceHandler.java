...
/**
 * The Class CreateModelicaRequirementInstanceHandler.
 */
public class CreateModelicaRequirementInstanceHandler extends AbstractHandler {
...
private String stereotypePath = Platform.getResourceString(Activator.getDefault().getBundle(), "%p_path_ModelicaRequirementConstructs");
	...
	/** The stereotyep name. */
	private String stereotyepName = Platform.getResourceString(Activator.getDefault().getBundle(), "%s_requirementInstance");
	...
	protected Command getCommand(TransactionalEditingDomain editingDomain) {
		...
		Command command = new RecordingCommand(editingDomain) {
			...
			protected void doExecute() {
				...
				Property py = null; // or it should be a component
				Stereotype stereotype = null;
				
//### START: adapt it for a new command handler
				if (selectedElement instanceof Class) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					py = ((org.eclipse.uml2.uml.Class)selectedElement).createOwnedAttribute(stereotyepName, null);
					stereotype = py.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				...
				if (stereotype == null) {
					...
					MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML stereotype " + stereotyepName +" to " + py.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
				}
				else {
