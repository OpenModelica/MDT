...
/**
 * The Class CreateModelicaModelHandler.
 */
public class CreateModelicaModelHandler extends AbstractHandler {
...
private String stereotypePath = Platform.getResourceString(Activator.getDefault().getBundle(), "%p_path_ModelicaClassConstructs");
	...
	/** The stereotyep name. */
	private String stereotyepName = Platform.getResourceString(Activator.getDefault().getBundle(), "%s_model");
...
//### START: adapt it for a new command handler
				if (selectedElement instanceof Class) {
					...
					clazz = (Class) ((org.eclipse.uml2.uml.Class)selectedElement).createNestedClassifier(stereotyepName, UMLPackage.Literals.CLASS);
					stereotype = clazz.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				else if (selectedElement instanceof Package) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					clazz = ((org.eclipse.uml2.uml.Package)selectedElement).createOwnedClass(stereotyepName, false);
					stereotype = clazz.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				else if (selectedElement instanceof Model) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					clazz = ((org.eclipse.uml2.uml.Model)selectedElement).createOwnedClass(stereotyepName, false);
					stereotype = clazz.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
				else {
