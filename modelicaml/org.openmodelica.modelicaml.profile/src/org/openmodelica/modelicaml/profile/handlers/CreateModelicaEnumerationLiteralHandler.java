...
/**
 * The Class CreateModelicaEnumerationLiteralHandler.
 */
public class CreateModelicaEnumerationLiteralHandler extends AbstractHandler {

//### START: adapt it for a new command handler
//	private String stereotypePath = Platform.getResourceString(Activator.getDefault().getBundle(), "%");
//	private String stereotyepName = Platform.getResourceString(Activator.getDefault().getBundle(), "%s_operator");
...
private EObject selectedElement = null;
	...
	protected Command getCommand(TransactionalEditingDomain editingDomain) {
		...
		CompoundCommand cc = new CompoundCommand("Enumeration Literal");
...
//### START: adapt it for a new command handler
				if (selectedElement instanceof Enumeration) {
					//TODO: Should the name get a post-fix number (e.g. ModelicaModel1, ModelicaModel2, etc.)?
					el = ((org.eclipse.uml2.uml.Enumeration)selectedElement).createOwnedLiteral("Enumeration Literal");
					//stereotype = P.getApplicableStereotype(stereotypePath+"::"+stereotyepName);
				}
...
//					MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML stereotype " + stereotyepName +" to " + P.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
//				}
//				else {
