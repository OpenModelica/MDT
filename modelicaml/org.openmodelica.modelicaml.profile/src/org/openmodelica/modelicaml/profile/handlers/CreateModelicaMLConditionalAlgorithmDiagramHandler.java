...
public class CreateModelicaMLConditionalAlgorithmDiagramHandler extends
	...
	private void applyStereotype(){
		if (behavior != null) {
			Stereotype s = behavior.getApplicableStereotype("ModelicaML::ModelicaBehaviorConstructs::ConditionalAlgorithm(Diagram)");
			if (s != null ) {
				behavior.applyStereotype(s);
			}
			else {
				Shell shell = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell();
				MessageDialog.openError(shell, "Error:", "Cannot apply ModelicaML stereotype 'ConditionalAlgorithm(Diagram)' to " + behavior.getName() + ". Please make sure that ModelicaML is applied to the top-level model/package.");
