 ...
 */
public class StateMachineSamplingExpressionSection extends AbstractPropertySection  {
	...
	public void createControls(Composite parent, TabbedPropertySheetPage tabbedPropertySheetPage) {
		...
		// TODO: create a new editor for the (sampling) expression
	...
	private Boolean isValidElement(){
		...
		if ( this.selectedUmlElement instanceof StateMachine) {
			return true;
		}
		// ################################ Adjust end
		return false;
	...
	public void refresh() {
		if (isNewSelection && isValidElement() ) { // Only react if a different (new) element was selected
			...
			if (selectedUmlElement instanceof StateMachine) {
				contextElement = ((StateMachine)selectedUmlElement);
				Stereotype stereotype = contextElement.getAppliedStereotype(Constants.stereotypeQName_ModelicaMLStateMachine);
				if (stereotype != null) {
					Object textObject = contextElement.getValue(stereotype, Constants.propertyName_samplingExpression);
					if (textObject instanceof String) {
						textToEdit = (String) textObject;
					}
 				}
		...
		if (element instanceof StateMachine) {
			Command command = new RecordingCommand(editingDomain) {
				...
				protected void doExecute() {
					Stereotype stereotype = selectedUmlElement.getAppliedStereotype(Constants.stereotypeQName_ModelicaMLStateMachine);
					if (stereotype != null) {
						selectedUmlElement.setValue(stereotype, Constants.propertyName_samplingExpression, bodyText);
					}
				}
			};
			cc.append(command);
	...
	private void generateUMLModelMarker(){
		...
		String message = "The " + ((NamedElement)selectedUmlElement).eClass().getName() 
							+ " '" + ((NamedElement)selectedUmlElement).getName() + "' has errors in its additional code.";
