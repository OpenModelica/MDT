 ...
 */
public class StateMachineFreezeExpressionSection extends AbstractPropertySection  {
	...
	public void refresh() {
		if (isNewSelection && isValidElement() ) { // Only react if a different (new) element was selected
			...
			if (selectedUmlElement instanceof StateMachine) {
				...
				if (stereotype != null) {
					Object textObject = contextElement.getValue(stereotype, Constants.propertyName_freezeExpression);
		...
		if (element instanceof StateMachine) {
			Command command = new RecordingCommand(editingDomain) {
				...
				protected void doExecute() {
					...
					if (stereotype != null) {
						selectedUmlElement.setValue(stereotype, Constants.propertyName_freezeExpression, bodyText);
					}
				}
			};
			cc.append(command);
